bl_info = {
    "name": "rFactor2 Suspension Model Exporter",
    "author": "FuNK!",
    "version": (0, 2, 2),
    "blender": (2, 90, 0),
...
class MassPanel(bpy.types.Panel):
    bl_label = "Mass"
    bl_idname = "OBJECT_PT_mass"
    ...
    def draw(self, context):
        ...
        row.label(text="Mass:")
        if obj:
            row.prop(obj, "mass", text="")
        else:
            row.label(text="No object selected.")

        row = layout.row()
        row.prop(context.scene, "mass_value")

        row = layout.row()
        row.operator("object.set_mass")

class MassPanel_SetMass(bpy.types.Operator):
    bl_label = "Set Mass"
    bl_idname = "object.set_mass"

    def execute(self, context):
        obj = context.object
        if obj:
            obj.data["mass"] = context.scene.mass_value
        return {'FINISHED'}

class SuspensionModelPMExporter(bpy.types.Operator, ExportHelper):
    """Exports the suspension definition of a vehicle into a PM suspension model file to be used in rFactor2."""
    ...
    def execute(self, context):
        ...
        try:
            with open(self.filepath, 'w') as pm_file:
                pm_file.write("// rFactor2 Suspension Model exported with the rFactor 2 Suspension Model Exporter Addon for Blender by FuNK!\n")
                pm_file.write("//////////////////////////////////////////////////////////////////////////\n")
                ...
                if not body_obj:
                    ...
                    body_obj_mass = body_obj.data.get("mass", 0.0) # retrieving the mass from the "mass" propetery of the object, set 0.0 if not available
                    pm_file.write("[BODY]\n")
                    if body_obj_mass == "0.0":
                        error_messages.append("Mass property not set for 'body' object, using defaults.\n")
                        pm_file.write("name=body mass=(0) inertia=(0,0,0)\n")
                        ...
                        inertia = [0.0, 0.0, 0.0]
                        inertia[0] = (body_obj_mass / 12.0) * (body_obj.dimensions[1]**2 + body_obj.dimensions[2]**2)
                        inertia[1] = (body_obj_mass / 12.0) * (body_obj.dimensions[0]**2 + body_obj.dimensions[2]**2)
                        inertia[2] = (body_obj_mass / 12.0) * (body_obj.dimensions[0]**2 + body_obj.dimensions[1]**2)
                        pm_file.write("name=body mass=({:.1f}) inertia=({:.3f}, {:.3f}, {:.3f})\n".format(body_obj_mass, inertia[0], inertia[1], inertia[2]))
                        pm_file.write("pos=({:.3f}, {:.3f}, {:.3f}) ori=({:.3f}, {:.3f}, {:.3f})\n\n".format(body_obj.location.x, body_obj.location.z, body_obj.location.y, body_obj.rotation_euler.x, body_obj.rotation_euler.z, body_obj.rotation_euler.y))
                ...
                if not body_obj:
                    ...
                    # Get mass property from the object
                    body_obj_mass = body_obj.data.get("mass", 0.0) # retrieving the mass from the "mass" property of the object, set 0.0 if not available

                    # Dimensions of the cylinder in all three axes
                    body_obj_diameter = body_obj.dimensions.x
                    body_obj_width = body_obj.dimensions.y
                    body_obj_height = body_obj.dimensions.z

                    # Calculate the moment of inertia
                    body_obj_inertia = [0.0, 0.0, 0.0]
                    body_obj_inertia[0] = (1/12) * body_obj_mass * (3*(body_obj_diameter**2) + body_obj_height**2)
                    body_obj_inertia[1] = (1/12) * body_obj_mass * (3*(body_obj_width**2) + body_obj_height**2)
                    body_obj_inertia[2] = (1/2) * body_obj_mass * (body_obj_diameter**2)

                    # Get object properties here and write to file
                    pm_file.write("// FRONT SPINDLES\n")
                    pm_file.write("[BODY]\n")
                    pm_file.write("name=fl_spindle mass=({:.1f}) inertia=({:.3f}, {:.3f}, {:.3f}) // Unsprung masses that do not exhibit any pitch rotation with significant relation to the wheel rotation rate\n".format(body_obj_mass, body_obj_inertia[0], body_obj_inertia[1], body_obj_inertia[2]))
                    pm_file.write("pos=({:.3f}, {:.3f}, {:.3f}) ori=({:.3f}, {:.3f}, {:.3f})\n\n".format(body_obj.location.x, body_obj.location.z, body_obj.location.y, body_obj.rotation_euler.x, body_obj.rotation_euler.z, body_obj.rotation_euler.y))
                ...
                if not body_obj:
                    ...
                    body_obj_mass = body_obj.data.get("mass", 0.0)
                    ...
                    pm_file.write("[BODY]\n")
                    pm_file.write("name=fr_spindle mass=({:.1f}) inertia=({:.3f}, {:.3f}, {:.3f})\n".format(body_obj_mass, body_obj_inertia[0], body_obj_inertia[1], body_obj_inertia[2]))
                    pm_file.write("pos=({:.3f}, {:.3f}, {:.3f}) ori=({:.3f}, {:.3f}, {:.3f})\n\n".format(body_obj.location.x, body_obj.location.z, body_obj.location.y, body_obj.rotation_euler.x, body_obj.rotation_euler.z, body_obj.rotation_euler.y))
                ...
                if not body_obj:
                    ...
                    pm_file.write("// FRONT WHEELS\n")
                    pm_file.write("[BODY]\n")
                    pm_file.write("name=fl_wheel mass=({:.1f}) inertia=({:.3f}, {:.3f}, {:.3f}) // Any unsprung bodies that rotate significantly in proportion to the wheel rotation rate (i.e. generally tyre+rim+disc/bell+bearing+nuts)\n".format(body_obj_mass, body_obj_inertia[0], body_obj_inertia[1], body_obj_inertia[2]))
                    pm_file.write("pos=({:.3f}, {:.3f}, {:.3f}) ori=({:.3f}, {:.3f}, {:.3f})\n\n".format(body_obj.location.x, body_obj.location.z, body_obj.location.y, body_obj.rotation_euler.x, body_obj.rotation_euler.z, body_obj.rotation_euler.y))
                ...
                if not body_obj:
                    ...
                    pm_file.write("[BODY]\n")
                    pm_file.write("name=fr_wheel mass=({:.1f}) inertia=({:.3f}, {:.3f}, {:.3f})\n".format(body_obj_mass, body_obj_inertia[0], body_obj_inertia[1], body_obj_inertia[2]))
                    pm_file.write("pos=({:.3f}, {:.3f}, {:.3f}) ori=({:.3f}, {:.3f}, {:.3f})\n\n".format(body_obj.location.x, body_obj.location.z, body_obj.location.y, body_obj.rotation_euler.x, body_obj.rotation_euler.z, body_obj.rotation_euler.y))
                ...
                if not body_obj:
                    ...
                    pm_file.write("// REAR SPINDLES\n")
                    pm_file.write("[BODY]\n")
                    pm_file.write("name=rl_spindle mass=({:.1f}) inertia=({:.3f}, {:.3f}, {:.3f})\n".format(body_obj_mass, body_obj_inertia[0], body_obj_inertia[1], body_obj_inertia[2]))
                    pm_file.write("pos=({:.3f}, {:.3f}, {:.3f}) ori=({:.3f}, {:.3f}, {:.3f})\n\n".format(body_obj.location.x, body_obj.location.z, body_obj.location.y, body_obj.rotation_euler.x, body_obj.rotation_euler.z, body_obj.rotation_euler.y))
                ...
                if not body_obj:
                    ...
                    pm_file.write("[BODY]\n")
                    pm_file.write("name=rr_spindle mass=({:.1f}) inertia=({:.3f}, {:.3f}, {:.3f})\n".format(body_obj_mass, body_obj_inertia[0], body_obj_inertia[1], body_obj_inertia[2]))
                    pm_file.write("pos=({:.3f}, {:.3f}, {:.3f}) ori=({:.3f}, {:.3f}, {:.3f})\n\n".format(body_obj.location.x, body_obj.location.z, body_obj.location.y, body_obj.rotation_euler.x, body_obj.rotation_euler.z, body_obj.rotation_euler.y))
                ...
                if not body_obj:
                    ...
                    pm_file.write("// REAR WHEELS (includes half of driveshaft)\n")
                    pm_file.write("[BODY]\n")
                    pm_file.write("name=rl_wheel mass=({:.1f}) inertia=({:.3f}, {:.3f}, {:.3f})\n".format(body_obj_mass, body_obj_inertia[0], body_obj_inertia[1], body_obj_inertia[2]))
                    pm_file.write("pos=({:.3f}, {:.3f}, {:.3f}) ori=({:.3f}, {:.3f}, {:.3f})\n\n".format(body_obj.location.x, body_obj.location.z, body_obj.location.y, body_obj.rotation_euler.x, body_obj.rotation_euler.z, body_obj.rotation_euler.y))
                ...
                if not body_obj:
                    ...
                    pm_file.write("[BODY]\n")
                    pm_file.write("name=rr_wheel mass=({:.1f}) inertia=({:.3f}, {:.3f}, {:.3f})\n".format(body_obj_mass, body_obj_inertia[0], body_obj_inertia[1], body_obj_inertia[2]))
                    pm_file.write("pos=({:.3f}, {:.3f}, {:.3f}) ori=({:.3f}, {:.3f}, {:.3f})\n\n".format(body_obj.location.x, body_obj.location.z, body_obj.location.y, body_obj.rotation_euler.x, body_obj.rotation_euler.z, body_obj.rotation_euler.y))
...
def register():
    ...
    bpy.utils.register_class(MassPanel)
    bpy.utils.register_class(MassPanel_SetMass)
    bpy.types.Scene.mass_value = bpy.props.FloatProperty(
        name="Value",
        default=0.0,
        min=0.0,
        max=10000.0,
...
def unregister():
    ...
    bpy.utils.unregister_class(MassPanel)
    bpy.utils.unregister_class(MassPanel_SetMass)
    del bpy.types.Scene.mass_value
